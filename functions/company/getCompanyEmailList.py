    ...
    try:
        ...
        for company in scanResponse['Items']:
            ...
            if "reporting" not in company:
                ...
                print(company["reporting"][reportingPeriod])
            ...
            emailHistory = getEmailData(companyData["id"], reportingPeriod)
            emailHistory.sort(key=lambda item : item["timestamp"], reverse=True)
            companyData["email"] = emailHistory
            companyData["comment"] = getComment(companyData["email"])
